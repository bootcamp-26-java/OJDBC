...
//    }
    ...
    public List<Job> getData(Object key, boolean IsGetById) {
        ...
        String query = (IsGetById)
                ...
                : "SELECT * FROM JOBS WHERE job_id LIKE(?) OR job_title LIKE(?) OR min_salary LIKE(?) OR max_salary LIKE(?)";
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(query);
            if (IsGetById) {
...
@Override
        ...
        ? "insert into jobs(job_title, min_salary, max_salary,job_id) values(?,?,?,?)"
